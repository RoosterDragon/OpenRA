	...
	public class LoadIngamePlayerOrObserverUILogic : ChromeLogic
	{
		...
		public LoadIngamePlayerOrObserverUILogic(Widget widget, World world)
		{
			...
			if (world.LocalPlayer == null)
				dynamicWidgets.LoadWidget(worldRoot, "OBSERVER_WIDGETS", []);
			else
			{
				var playerWidgets = dynamicWidgets.LoadWidget(worldRoot, "PLAYER_WIDGETS", []);
				...
				sidebarTicker.OnTick = () =>
				{
					...
					if (world.LocalPlayer.WinState != WinState.Undefined && !loadingObserverWidgets)
					{
						...
						Game.RunAfterDelay(objectives?.GameOverDelay ?? 0, () =>
						{
							...
							dynamicWidgets.LoadWidget(worldRoot, "PLAYER_WIDGETS", []);
						});
					}
				};
			}

			dynamicWidgets.LoadWidget(ingameRoot, "DEBUG_WIDGETS", []);
			dynamicWidgets.LoadWidget(ingameRoot, "TRANSIENTS_PANEL", []);
